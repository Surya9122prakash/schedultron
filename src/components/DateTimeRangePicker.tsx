...
import type { DateTimeRangePickerProps, RangeValue } from "./types";
...
export const DateTimeRangePicker = forwardRef<HTMLInputElement, DateTimeRangePickerProps>(({
    ...
    const displayValue = range.start ? `${range.start.format(fullFormat)} - ${range.end ? range.end.format(fullFormat) : "..."}` : "";
                ...
                <div className="absolute z-50 mt-2 bg-white rounded-xl shadow-xl overflow-hidden animate-fadeIn" style={{ backgroundColor: "var(--calendar-secondary-bg, #f9fafb)", border: "1px solid var(--calendar-grid, #e5e7eb)" }}>
                    <CalendarPanel
                        ...
                        onChange={(val: RangeValue) => {
                            // Preserve times when changing dates
                            const newStart = val.start?.clone().hour(range.start?.hour() || 0).minute(range.start?.minute() || 0) || null;
                            const newEnd = val.end?.clone().hour(range.end?.hour() || 23).minute(range.end?.minute() || 59) || null;
                    ...
                    />
                    <div className="p-4 border-t flex flex-col gap-3" style={{ borderColor: "var(--calendar-grid, #e5e7eb)" }}>
                        <div className="flex items-center gap-3">
                            <div className="flex-1">
                                <label className="text-[10px] font-bold uppercase opacity-50 mb-1 block" style={{ color: "var(--calendar-text, #000)" }}>Start Time</label>
                                <input
                                    type="time"
                                    value={range.start?.format("HH:mm") || ""}
                                    onChange={(e) => {
                                        const [h, m] = e.target.value.split(":");
                                        const newStart = (range.start || moment.tz(timezone)).clone().hour(parseInt(h)).minute(parseInt(m));
                            ...
                            <div className="flex-1">
                                <label className="text-[10px] font-bold uppercase opacity-50 mb-1 block" style={{ color: "var(--calendar-text, #000)" }}>End Time</label>
                                <input
                                    type="time"
                                    value={range.end?.format("HH:mm") || ""}
                                    onChange={(e) => {
                                        const [h, m] = e.target.value.split(":");
                                        const newEnd = (range.end || moment.tz(timezone)).clone().hour(parseInt(h)).minute(parseInt(m));
                        ...
                        <button
                            ...
                            className="w-full mt-1 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity"
                        >
                            Done
                        </button>
                    </div>
